...
| 2 | 配置文件位置 | 主推全局配置（macOS `~/Library/Application Support/git-notes-sync/config.toml`、Linux `~/.config/git-notes-sync/config.toml`、Windows `%APPDATA%\git-notes-sync\config.toml`，经 `os.UserConfigDir()`）+ `gns repos add/del` 维护多仓库，一般无需仓库级配置；仓库根 `.notes-sync.toml` 作为可选覆盖（仓库 > 全局 > 默认）；另有 `-c` 显式指定 |
