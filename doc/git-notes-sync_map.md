...
- **适度健壮**：保护本机文件、Git 历史和 `.syncable` 等关键边界；无关紧要的输入差异和可恢复异常采用统一默认行为，不为它们增加大量分支和专用状态。
- **避免健壮性损害性能**：copy 模式只执行同步本身需要的目录遍历。并发变化检查复用复制过程中已经读取的 size 和 mtime，不额外执行全树扫描；普通 `status` 也不扫描映射目录内容。
- **异常阻断并交给用户**：文件类型、映射方向或 Git 历史无法确定时，停止当前链路；只有确定需要人工重新选择内容时才删除 `.syncable`，其余错误直接报告并留待下次重试。
...
一次同步中，同一方向的每个映射最多执行一次必要的全树遍历。自动同步在本机内容上行时顺便记录 size 和 mtime，远程内容下发前直接复用这些元数据检查同步期间的本机变化，不增加独立的 TOCTOU 全树扫描。合并没有改变当前机器的映射路径时跳过下发遍历。`gnm status` 默认只检查映射根和 Git 状态，不递归比较本机与 worktree 内容。
