...
> `map` 功能需求与设计草案。
>
> 本文描述目标、数据模型、命令职责和同步流程。命令名称及部分配置字段仍可在实现前调整，但安全边界和处理原则应保持不变。
...
| 维度 | 说明 |
|------|------|
| 适用场景 | 多机器 dotfile / config / skill / 脚本 / 目录的统一管理与同步 |
| 运行方式 | 复用 GNS 现有调度（cron / systemd / launchd / daemon），不单独注册定时任务 |
| 与 sync 的关系 | 职责独立：`map` 管本机文件映射，遇冲突阻断由人工解决；`sync` 管普通仓库同步，自动处理冲突 |
| 冲突策略 | 唯一冲突点（worktree merge git-root）需人工解决；网络等异常不伪装为冲突（见 §2.3） |
| 文件一致性 | link 模式实时强一致；copy 模式按命令 / 定时增量收敛（最终一致） |
| 安全闸门 | `.syncable` 控制自动同步开关；首次及阻断恢复后需人工确认（见 §3.2） |

### 1.3 非目标

- **不做文件 watcher**：copy 模式用增量扫描，行为可预测（见 §5.3）。
- **不自动语义合并冲突**：冲突由用户通过 `gnm add` / `gnm get` 人工选择方向。
- **worktree 禁止 hard reset**：会覆盖正在使用的真实文件（见 §8.1）。
- **不管理多 map-root 并发**：状态按 map-root 隔离，但本文不展开并发控制细节。
...
- **简单处理边界**：只自动处理方向明确、结果确定的步骤；边界状态无法可靠判断时立即停止并给出人工操作，不为低概率场景引入复杂状态机或自动猜测。
- **适度健壮**：优先保护本机文件、Git 历史和 `.syncable`；自动流程只处理结果确定的步骤并复用必要遍历，无法判断时阻断并交给用户，不为低概率边界增加复杂状态或额外扫描。只有确认需要重新选择内容或历史时才删除 `.syncable`，其余错误留待下次重试；完整诊断由用户显式执行 `gnm status`。
...
- git-root 是远程当前分支的本机集成副本，不承载独立的机器修改。
- 定时 `pull` 负责把远程状态更新到 git-root。
- 正常自动流程只允许 fast-forward；一旦无法 fast-forward 则阻断，由用户通过 `gnm pull --force` 强制以远程分支恢复一致（见 §8.1）。
- 网络失败只会推迟本次同步，不形成需要人工选择内容的冲突。

**worktree 文件 ↔ 本机真实文件**

- 软链接模式下，两者实际指向同一份内容。
- copy 模式下，GNS 按命令或定时任务执行确定方向的覆盖并最终收敛（规则见 §5.3）。
- 正常提交前执行"本机 → worktree"；Git 内容下发后执行"worktree → 本机"。
...
worktree 成功合并 git-root 后，worktree 已包含 git-root 的全部历史，因此后续 `git-root merge worktree` 只能 fast-forward，不再执行内容合并。fast-forward 失败属于异常：报告错误、删除 `.syncable`（见 §3.2）并停止，等待用户处理。

> 映射根仅在一侧存在属于删除安全检查（见 §5.2），不是 Git 内容冲突——它同样要求用户显式选择，但不会产生 conflict markers。
...
`map` 只维护三个核心状态，运行时只需保存必要的 worktree 信息、锁和 `.syncable`，不维护复杂的多端同步数据库。

### 3.1 三状态定义

| 状态 | 条件 | 允许的操作 |
|------|------|------------|
| `NOT_INITIALIZED` | worktree 尚未创建 | 配置、`gnm init` |
| `MANUAL_REQUIRED` | worktree 已创建，但不存在 `.syncable` | status、pull、add、get、commit、手动 push |
| `SYNCABLE` | 存在 `.syncable` | 手动操作和定时 `gnm sync` |
...
- 映射根仅在本机或 worktree HEAD 一侧存在，需要用户显式选择（见 §5.2）；
- 有效映射配置发生变化（`add` / `remove` / `load`）；
- 其他已经确认无法在不选择内容或历史的前提下继续的异常。

**异常保留语义**：`.syncable` 表示"当前内容选择和 Git 历史仍被信任"，不表示每次运行都必须成功。断网、DNS、超时、认证或权限失败、map 锁竞争、Git 正在 merge / rebase / cherry-pick，以及 push 被拒绝，都只终止本次任务并**保留** `.syncable`。它们不足以证明内容或历史已经需要人工选择，下一次同步仍从 `git-root pull --ff-only` 开始重试；只有届时确认无法 fast-forward，才删除标记并进入 `MANUAL_REQUIRED`。
...
```text
gnm config ...   # 定义映射关系、保存和加载配置
gnm ...          # 初始化、提交、同步和阻断恢复
```

`gnm config` 负责映射定义及单项映射的建立和移除，不创建整个 worktree；`gnm` 的其他命令负责 worktree、本机文件和 git-root 的整体状态流转。worktree 尚未初始化时，`config add` / `remove` 只修改用户配置；初始化后，它们会立即应用对应映射的建立或移除逻辑（见 §4.5）。
...
> 两侧都不存在表示当前内容一致，`status` 只展示该事实，不要求创建空文件，也不阻止首次手动 `push`。以后任意一侧出现内容时，再按映射根仅一侧存在的规则要求用户执行 `gnm add` 或 `gnm get`（见 §5.2）。

`gnm config remove` 执行单项 init 的反向操作，确保解除映射后本机仍保留可直接使用的真实文件。仓库内容是否删除由 scope 决定：
...
初始化后的 `add` / `remove` 都会删除 `.syncable`（见 §3.2）。操作完成后由 `gnm status` 展示结果，用户确认并成功执行 `gnm push` 后才恢复自动同步。
...
- `load`：从 git-root 当前 HEAD 中的 `.gns/map/<有效 map-root>.toml` 读取机器配置快照并导入用户配置，不依赖 worktree 已经创建。只允许在 worktree 尚未初始化时执行，只加载配置，不创建 worktree，也不覆盖本机文件；加载后由首次 `gnm init` 应用全部映射。worktree 已初始化时执行 `load` 应报错并保持现状，后续映射调整使用会立即应用的 `gnm config add` / `remove`。
...
> 以下规则为 `gnm add` / `gnm get` / `gnm sync` 共享，各命令只在此基础上体现差异（见 §6.5–6.8）。

### 5.1 路径与通配符

- 参数是具体本机文件、目录或包含 `*` 的路径模式。相对路径按当前工作目录解析，也可使用绝对路径或 `~/`。目录参数递归处理其内容。
- 通配符由 GNS 跨平台展开。`*` 匹配单个路径层级内的任意名称，包括以 `.` 开头的名称，但不跨越路径分隔符；匹配到目录后递归处理整个目录。
- 匹配范围只限已配置映射的本机路径及其子路径，不会处理未映射文件。
- `add` 使用本机与 worktree 文件的并集进行匹配，因此本机已删除、但 worktree 中仍存在的文件也能被模式选中并暂存删除。`get` 使用本机与 HEAD 文件的并集进行匹配。
- 模式没有匹配到任何映射内容时应报错，不静默成功。命令示例推荐给模式加引号，避免不同 shell 提前展开产生不一致行为。
...
映射根检查通过、或用户已通过 `add` / `get` 明确方向后，文件过滤规则为：

1. 目标不存在或文件类型不同：执行复制或替换；
2. 文件大小不同：执行复制；
3. 文件大小相同但 mtime 不同：执行复制；
4. 文件大小和 mtime 相同但权限不同：执行复制；
5. 文件大小、mtime 和权限都相同：跳过，不读取文件内容，也不计算 hash。
...
一次同步中，同一方向的每个映射最多执行一次必要的全树遍历。自动同步在本机内容上行时顺便记录 size、mtime 和权限，远程内容下发前直接复用这些元数据检查同步期间的本机变化，不增加独立的 TOCTOU 全树扫描。合并没有改变当前机器的映射路径时跳过下发遍历。用户显式执行 `gnm status` 时，允许递归扫描本机与 worktree，以展示尚未执行 `add` 或定时同步的 copy 侧变化。

### 5.4 文件类型与权限处理

- **普通文件**：复制内容、mtime 和文件权限；Linux / macOS 至少必须保留 executable bit，Windows 忽略文件系统不支持的 Unix 权限位。
- **目录**：递归完成后同步目录权限和 mtime；目录 mtime 不用于判断子内容是否变化。
- **软链接**：映射目录内部的软链接按软链接本身复制，不跟随链接目标，避免越界复制和目录循环；比较软链接时比较其目标路径。当前平台无法创建对应软链接时报告错误并终止当前命令，不静默改为复制链接目标。
- **特殊文件**：socket、FIFO、设备文件等不复制，报告具体路径并终止当前命令；自动同步发生此类错误时删除 `.syncable`，进入 `MANUAL_REQUIRED`。
...
`gnm` 是 map 功能的短命令入口，应由安装产物直接提供，并在程序入口完成命令展开，不依赖用户自行配置 shell alias。这样 Bash、PowerShell、Windows 任务计划和 daemon 都使用同一套命令行为。

```text
gnm config       = gns map-config
gnm config add … = gns map-config add …
gnm <command>    = gns map <command>   （config 以外）
```
...
```text
 ...
 3. gnm config map-root <name>，或通过 gnm config load <map-root> 加载已有配置
 4. gnm config add ...
 5. gnm init
 6. gnm config save
...
2. 从 git-root 当前 HEAD 创建本地 `gns/map/<map-root>-worktree` 分支（见 §2.4）；
3. 在 GNS 应用目录创建 worktree；
4. 建立软链接或执行首次 copy（文件处理规则见 §4.5 表格）；
5. 输出初始化结果；
...
**link 模式初始化**：始终先从 git-root 当前 HEAD 创建 worktree，再处理本机文件。初始化不会移动 worktree HEAD，也不执行 `git add` 或 commit：
...
worktree staged / unstaged / untracked 状态
copy 模式本机内容是否尚未刷新到 worktree
是否发生 merge 阻断
.syncable 是否存在
每个映射是否存在、类型是否匹配
```

`status` 只使用 §3.1 定义的三个核心状态，其他信息作为原因和详情展示。任何状态下，`status` 都必须给出可直接执行的下一步命令，而不只展示状态。主要组合的建议如下：
...
软链接模式主要对对应 worktree 路径执行 `git add`；copy 模式先把指定本机内容增量复制到 worktree（规则见 §5.3），再暂存对应路径。路径在本机已经不存在时，`add` 表示确认并暂存对应 worktree 内容的删除。`-A` / `--all` 对所有映射选择本机版本，最后对整个 worktree 执行 `git add -A`，因此也会暂存 `gnm config save` 写入的 `.gns/map/*.toml` 配置快照。

**`gnm get`** — 选择 worktree 当前 HEAD 中的版本并下发本机：
...
将 HEAD 版本同时写入 index 和 worktree 文件，因此也会清除该路径之前已暂存或未暂存的差异。软链接模式下本机文件随 worktree 文件同步恢复；copy 模式恢复 worktree 文件后再复制到本机。操作完成后，该路径的 index、worktree 和本机文件都与 HEAD 一致，不产生需要提交的修改。
...
如果仍有未提交或未暂存修改，则停止并由 `status` 给出建议。

完整成功后创建或恢复 `.syncable`（见 §3.2）。

集成链路与 `gnm sync` 共用（见 §7.2），区别只在进入链路前要求用户已完成选择和提交。

### 6.8 `gnm sync`

`gnm sync` 是 map 的自动同步入口，必须在 `.syncable` 存在时运行。内部自动执行 `git-root pull → 映射根预检 → worktree add -A → commit → worktree merge git-root → 合并后映射根复检 → git-root fast-forward 到 worktree HEAD → git-root push`（完整流程见 §7.2）。

映射根需要人工选择或没有 `.syncable` 时，只输出推荐的处理命令，不继续修改文件或 Git 状态。

### 6.9 `gnm pull`

`pull` 不是普通的无条件下发命令，而是发生阻断后进入人工恢复流程的入口（完整流程见 §8.1）。

- `gnm pull`：要求 git-root 能够 fast-forward。
- `gnm pull -f` / `--force`：丢弃 git-root 与远程分支的历史分叉，强制以远程分支更新 git-root。

两者都会把 worktree HEAD 和 index 更新到 git-root 新基准，但保证 worktree 文件和本机真实文件不变。
...
- `gnm push`：要求用户已经通过 `gnm add` / `get` / `commit` 完成选择；如果仍有未提交或未暂存修改，则停止。
- `gnm sync`：确认 `.syncable` 存在后，自动执行 `gnm add -A` 并创建默认提交，然后进入集成链路。

```text
 1. 获取 map 锁
 2. git-root pull --ff-only，与远程保持一致
    → 确认无法 fast-forward 时删除 .syncable（见 §3.2）
    → 其他异常保留 .syncable
    → 均停止本次运行
 3. 检查本机与 worktree 的映射根（见 §5.2）
    → 仅一侧存在时删除 .syncable，进入 MANUAL_REQUIRED
 4. sync 模式：确保本机文件与 worktree 文件一致，自动 add -A + commit
    push 模式：确认 worktree 已经由用户完成选择和提交
 5. worktree merge git-root 当前 HEAD        ← 唯一冲突点（见 §2.3）
 6. 若冲突：恢复合并前的 worktree 文件，删除 .syncable，停止（见 §7.3）
 7. 合并成功后再次检查映射根（见 §5.2）
    → 仅一侧存在时恢复合并前状态，删除 .syncable，进入 MANUAL_REQUIRED
 8. 把合并后的 worktree 内容下发到本机
 9. git-root fast-forward 到 worktree HEAD
    → 失败则报错、删除 .syncable 并停止
10. git-root push
    → 失败时保留 .syncable 并停止，由下一次 pull --ff-only 判定是否已发生历史分叉
...
只有第 5 步允许进入 Git 内容冲突状态（见 §2.3）。发生 Git 冲突时：
...
- 由 `gnm status` 输出冲突文件和完整恢复步骤，等待用户执行 `gnm pull`（见 §8）。
...
```text
 ...
 7. 用户按文件选择保留本机内容或 git-root HEAD 内容（见 §8.2）
...
整个处理期间 `.syncable` 始终不存在（见 §3.2）。只有最后一次手动 `gnm push` 完整成功，才重新创建它。
...
2. **唯一内容冲突点**：只在 worktree merge git-root 时进入人工阻断（见 §2.3）。
3. **自动同步有闸门**：没有 `.syncable` 就不能执行 `gnm sync`，也不会自动 commit 或 push（见 §3.2）。
4. **worktree 禁止 hard reset**：切换 worktree 基准只使用保留文件的 `reset --mixed`；`--force` 只能对专用 git-root 使用 `reset --hard`（见 §8.1）。
...
9. **异常不伪装成冲突**：网络、认证、权限、锁竞争、正在进行的 Git 操作和 push 拒绝都只终止当次运行并保留 `.syncable`；只有确认内容需要选择或历史无法 fast-forward 时才删除（见 §3.2）。
10. **copy 不做 watcher**：命令执行时增量扫描并复制，行为可预测（见 §5.3）。
