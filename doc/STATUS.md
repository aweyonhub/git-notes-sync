...
| P2 | map 映射 ignore 规则 | 映射目录时按规则忽略子项——如 `.codex/skills` 里的 `.system`（codex 系统内置 skill）不映射，只同步 `vip-token-pi` 等自定义项。设计要点：`[[map.items]]` 增加 ignore 字段（glob 模式，类 `.gitignore` 语义）；copy/link 模式的 SyncTree 遍历与删除传播跳过匹配项；`gnm status` 对忽略项不报差异 |
