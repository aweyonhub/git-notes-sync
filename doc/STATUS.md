...
- GitHub Actions：tag 触发交叉编译 6 平台 + 发布 Release；CI 增强：gofmt 校验 + `-race`（ubuntu 常驻）、windows-latest 真机测试（tag/手动 `workflow_dispatch` 触发，并入发布门槛 `build.needs`）、genversion 一致性校验
...
| P1 | Windows 实机验证 | ✅ daemon 行为、credential helper 继承已验；autocrlf 已覆盖（`TestSyncAutocrlfInherited`：windows-test 设 global `core.autocrlf=true` 验归一化分支，ubuntu 验 unset 分支；v0.1.4 已含 wscript + .vbe 隐藏窗口方案，见下） |
...
| P0 | map 映射 ignore 规则 | 映射目录时按规则忽略子项——如 `.codex/skills` 里的 `.system`（codex 系统内置 skill）不映射，只同步 `vip-token-pi` 等自定义项。设计要点：`[[map.items]]` 增加 ignore 字段（glob 模式，类 `.gitignore` 语义）；copy/link 模式的 SyncTree 遍历与删除传播跳过匹配项；`gnm status` 对忽略项不报差异。详见 `STATUS_MAP.md` §五 |
