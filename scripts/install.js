...
// GitHub release downloads redirect (302) to a CDN; Node does not follow
// redirects by default, so we do it manually.
function download(currentUrl, redirectsLeft) {
  clientFor(currentUrl)
    .get(currentUrl, (res) => {
      const loc = res.headers.location;
      if (res.statusCode >= 300 && res.statusCode < 400 && loc && redirectsLeft > 0) {
        res.resume(); // drain
        return download(new URL(loc, currentUrl).toString(), redirectsLeft - 1);
      }
      if (res.statusCode !== 200) {
        console.error(`git-notes-sync: download failed (HTTP ${res.statusCode}) from ${currentUrl}`);
        process.exit(1);
      }
